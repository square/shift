...
var generatePtOscCommandTests = []struct {
	...
	{"defaults-file.cnf", "/log/path/", false, &migration.Migration{Id: 7, DdlStatement: "alter table    t1 drop column c",
		...
		CustomOptions: map[string]string{}},
		[]string{"--alter", "drop column c", "--execute", "-h", "rw.hostname", "--defaults-file", "defaults-file.cnf",
			"--progress", ptOscProgress, "--exit-at", "copy", "--save-state", stateFile, "--max-load", "Threads_running=125",
			"--critical-load", "Threads_running=200", "--tries", "create_triggers:200:1,copy_rows:10000:1", "--max-lag", "1",
			"--set-vars", "wait_timeout=600,lock_wait_timeout=1", "D=db,t=t"}},
	...
	{"defaults-file.cnf", "/log/path/", true, &migration.Migration{Id: 7, DdlStatement: "alter table    t1 drop column c",
		...
		[]string{"--alter", "drop column c", "--execute", "-h", "rw.hostname", "--defaults-file", "defaults-file.cnf",
			...
			"--max-load", "Threads_running=125", "--critical-load", "Threads_running=200", "--tries",
			"create_triggers:200:1,copy_rows:10000:1", "--max-lag", "1", "--set-vars", "wait_timeout=600,lock_wait_timeout=1", "D=db,t=t"}},
	...
	{"defaults-file.cnf", "/log/path/", false, &migration.Migration{Id: 7, DdlStatement: "alter table    t1 drop column c",
		...
		CustomOptions: map[string]string{}},
		[]string{"--alter", "drop column c", "--execute", "-h", "rw.hostname", "--defaults-file", "defaults-file.cnf",
			"--progress", ptOscProgress, "--exit-at", "copy", "--save-state", stateFile, "--nocheck-alter", "--max-load",
			"Threads_running=125", "--critical-load", "Threads_running=200", "--tries", "create_triggers:200:1,copy_rows:10000:1",
			"--max-lag", "1", "--set-vars", "wait_timeout=600,lock_wait_timeout=1", "D=db,t=t"}},
	...
	{"defaults-file.cnf", "/log/path/", false, &migration.Migration{Id: 7, DdlStatement: "alter table    t1 drop column c",
		...
		[]string{"--alter", "drop column c", "--execute", "-h", "rw.hostname", "--defaults-file", "defaults-file.cnf",
			"--progress", ptOscProgress, "--exit-at", "copy", "--save-state", stateFile, "--max-load", "Threads_running=125",
			"--critical-load", "Threads_running=321", "--tries", "create_triggers:200:1,copy_rows:10000:1", "--max-lag", "4",
			"--set-vars", "wait_timeout=600,lock_wait_timeout=1", "D=db,t=t"}},
	...
	{"defaults-file.cnf", "/log/path/", false, &migration.Migration{Id: 7, DdlStatement: "alter table    t1 drop column c",
		...
		[]string{"--config", "/path/to/file", "--alter", "drop column c", "--execute", "-h", "rw.hostname", "--defaults-file", "defaults-file.cnf",
			"--progress", ptOscProgress, "--exit-at", "copy", "--save-state", stateFile, "--recursion-method", "hosts", "--max-load", "Threads_running=125",
			"--critical-load", "Threads_running=321", "--tries", "create_triggers:200:1,copy_rows:10000:1", "--max-lag", "4",
			"--set-vars", "wait_timeout=600,lock_wait_timeout=1", "D=db,t=t"}},
